...
//   return (
//     <div className="stories mt-8 md:mt-25 border-0 border-b-1 h-47 flex gap-8 items-center p-5 py-10 overflow-x-auto scroll-snap-x snap-mandatory scroll-smooth w-full">
//       {type === "weeks" ? (
//         weeks.map((week, i) => (
//           <Story
//             type={type}
...
const Stories = ({ type }) => {
  ...
  const handleScroll = () => {
    ...
    setAtStart(scrollLeft >= maxScrollLeft - 1); // اقصى اليسار → بداية القائمة (لأن rtl)
    setAtEnd(scrollLeft <= 1); // اقصى اليمين → نهاية القائمة
  };
  ...
  // في rtl:
  // الزر اليمين: scroll content left (نقل المحتوى لليسار)
  // الزر اليسار: scroll content right (نقل المحتوى لليمين)
  const scrollLeft = () => {
