...
export default function EditStory({ story }: { story: Story }) {
  const [title, setTitle] = useState<string>(story.title);
  const [summary, setSummary] = useState<string>(story.summary ?? "");
  const [year, setYear] = useState<string>(String(story.year ?? ""));
  const [count, setCount] = useState<number>(story.count);
  const [type, setType] = useState(story.type);
  const [special, setSpecial] = useState<boolean>(story.special ?? false);
